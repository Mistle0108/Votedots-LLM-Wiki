...
- canvas 관련 API는 현재 canvas, 체크 범위, 참여자 목록, 게임 요약, 광장 현재 게임 조회 흐름과 연결된다.
...
- 모바일 캔버스 입력 보정, floating palette, 하단 sheet, 결과 다운로드 연결이 추가돼 있다.

## 주요 경로
| 계층 | 경로 | 사실 |
| --- | --- | --- |
| Frontend | `frontend/src/features/gameplay/canvas/*` | canvas container, viewport, 좌표 이동, 체크 로딩, 미니맵, interaction 관련 UI가 위치한다. |
| Frontend | `frontend/src/pages/canvas/*` | CanvasPage와 scene/page model이 canvas UI를 summary/tutorial/room context와 조합한다. |
| Frontend | `frontend/src/shared/hooks/useSnapshotDownload.ts` | 결과 이미지 다운로드 연결을 담당한다. |
