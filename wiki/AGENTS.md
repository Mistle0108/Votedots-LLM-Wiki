...
- 프로젝트 세션에서 `wiki` 요청의 bootstrap과 요청 해석 기준 진실은 프로젝트 저장소의 `AGENTS.md`에 둔다.
- 이 파일은 연결된 wiki 저장소가 확정된 뒤 `wiki/` 내부 읽기/쓰기/publish의 상세 절차를 정의한다.
...
- 프로젝트 세션에서 `wiki` 요청을 어떻게 해석할지와 wiki 경로 bootstrap 기준 진실은 프로젝트 저장소의 `AGENTS.md`를 따른다.
- 이 절은 연결된 wiki 저장소가 확정된 뒤, 이 저장소에서 어떤 순서로 읽기와 반영 작업을 진행할지 정의한다.
