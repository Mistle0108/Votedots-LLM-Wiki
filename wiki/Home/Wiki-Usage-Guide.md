...
- 프로젝트 세션에서 local wiki 저장소 경로를 확인할 때는 먼저 `raw/repos/{repo}.local.md`의 `wiki_repo_path`를 사용한다.
- `wiki_repo_path`는 프로젝트 세션의 shell에서 실제로 접근 가능한 wiki 경로여야 한다. 프로젝트 `execution_path`가 WSL이면 wiki 경로도 WSL에서 보이는 경로를 쓴다.
- `wiki_repo_path`가 없거나 접근할 수 없으면 LLM은 경로를 한 번만 확인하고 같은 파일에 저장한 뒤, 이후 기본값으로 재사용한다.
...
- `wiki_repo_path`가 없거나 현재 세션에서 접근할 수 없으면 LLM은 자동 추정하지 않고 경로를 한 번만 확인한 뒤 같은 파일에 저장해야 한다.
...
```text
프로젝트 세션에서 local card의 `wiki_repo_path`로 local wiki repo 경로를 먼저 확인하고,
로컬 wiki와 origin/main이 같은지 확인해줘.
...
```text
프로젝트 세션에서 local card 기준 local wiki 저장소를 먼저 확인하고,
wiki 최신 상태를 확인한 뒤,
