...
- 사용자의 명령을 wiki 기준으로 수행하기 전에는 `git fetch origin main`으로 원격 최신 상태를 다시 확인하고 `origin/main@<hash>`를 기준 ref로 확정한다.
...
- 프로젝트 세션에서는 로컬 wiki를 참조 문서로 읽고, 사용자의 각 명령이나 중요한 판단 단계 전에 `git fetch origin main`으로 기준 `origin/main` 최신 ref를 다시 확인한다.
...
- `wiki/AGENTS.md`는 같은 세션에서 처음으로 wiki 읽기/쓰기/등록 판단에 들어갈 때 1회 읽는다.
- 같은 세션에서 이미 읽은 `AGENTS.md` 계열 문서는 매 요청마다 전체를 다시 읽지 않는다.
- 단순 후속 요청이나 같은 작업 흐름 안에서는 이미 확인한 AGENTS 규칙을 세션 기준값으로 재사용한다.
- 다시 읽기가 필요한 경우는 저장소 변경, 브랜치 변경, 사용자가 규칙 변경을 요청한 경우, 작업 범위가 다른 규칙 영역으로 넘어간 경우, 현재 세션에서 `AGENTS.md` 계열 문서를 수정한 경우, 규칙 충돌이나 해석 불확실성으로 현재 판단이 막히는 경우로 한정한다.
- 최신 상태처럼 시간에 따라 바뀔 수 있는 정보 확인과 `AGENTS.md` 재읽기는 구분한다. 새 요청이나 중요한 판단 전에는 `git fetch origin main`으로 기준 ref를 다시 확인할 수 있지만, 그때마다 `AGENTS.md` 전체를 재읽는 것을 기본 경로로 쓰지 않는다.
...
- wiki 반영 직전에는 `git fetch origin main`으로 기준 `origin/main@<hash>`를 다시 확정한다.
...
## 간단한 GitHub 작업 빠른 처리 기준
- `이슈 생성`, `작업 브랜치 생성`, `이슈 + 작업 브랜치 준비`처럼 문서 수정이 없는 요청은 빠른 경로로 처리한다.
- 같은 세션에서 이미 확인한 저장소 대상, issue template, 브랜치 규칙, 기준 commit은 다시 읽지 않는다. 다시 확인이 필요한 것은 원격 상태처럼 시간에 따라 바뀔 수 있는 정보만 한정한다.
- `이슈만` 요청이면 대상 저장소와 해당 저장소 issue template만 확인한 뒤 바로 생성한다.
- `작업 브랜치만` 요청이면 `git fetch origin main` 후 `origin/main` 기준으로 바로 브랜치를 만든다.
- `이슈 + 작업 브랜치` 요청이면 template 1회 확인, 이슈 생성, `git fetch origin main`, 브랜치 생성 순서로 처리한다.
- 단순 GitHub 작업을 이유로 전체 wiki 읽기, 관련 없는 문서 재탐색, 중복 규칙 확인을 기본 경로로 사용하지 않는다.
- 대상 저장소가 불명확하거나, template이 여러 개이거나, 원격 확인이 실패한 경우에만 추가 탐색을 한다.
